# 付録A 一般部位及び大部分がガラスで構成されていないドア等の開口部における日よけの効果係数
...
def get_gamma(y1, y2, gamma1, gamma2):
    """一般部位及び大部分がガラスで構成されていないドア等の開口部における日よけの効果係数 (-)

    Args:
      y1(float): 日よけ下端から一般部及び大部分がガラスで構成されていないドア等の開口部の上端までの垂直方向距離 (mm)
      y2(float): 一般部及び大部分がガラスで構成されていないドア等の開口部の高さ寸法 (mm)
      gamma1(float): データ「日よけの効果係数」より算出した値
      gamma2(float): データ「日よけの効果係数」より算出した値

    Returns:
      float: 一般部位及び大部分がガラスで構成されていないドア等の開口部における日よけの効果係数
...
def get_l1(y1, z):
    ...
    Args:
      y1(float): 日よけ下端から一般部及び大部分がガラスで構成されていないドア等の開口部の上端までの垂直方向距離 (mm)
      z(float): 壁面からの日よけの張り出し寸法（軒等の出寸法は壁表面から先端までの寸法とする）(mm)
...
def get_blind_gamma_f(region, l, H_or_C, direction):
    """lをパラメーターとして地域の区分及びガラスの仕様の区分に応じ、データ「日よけの効果係数」より算出した値
