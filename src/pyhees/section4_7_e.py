...
#       （給湯熱源：ガス瞬間式、暖房熱源：電気ヒートポンプ・ガス瞬間式併用、貯湯タンクあり）
# ============================================================================
...
def calc_E_E_hs(Q_dmd_H_hs, Theta_ex, Theta_RW_hs, h_ex, Theta_SW_d_t, TU_place):
    """1時間当たりの温水暖房用熱源機の消費電力量 (1)

    Args:
      Q_dmd_H_hs(ndarray): 1時間当たりの熱源機の熱需要 (MJ/h)
      Theta_ex(ndarray float): 外気温度
      Theta_RW_hs(ndarray): 1時間平均の温水暖房用熱源機の戻り温水温度 (℃)
      h_ex(ndarray): 外気相対湿度
      Theta_SW_d_t(ndarray): 往き温水温度 (℃)
      TU_place(str): タンクユニットの設置場所

    Returns:
      ndarray: 1時間当たりの温水暖房用熱源機の消費電力量
...
def calc_E_G_hs(Theta_ex, Theta_SW_d_t, Theta_RW_hs, TU_place, Q_dmd_H_hs):
...
def get_Theta_HPU_out(r_HPU, Theta_SW_hs):
    ...
    Args:
      r_HPU(ndarray): 1時間平均の温水暖房用熱源機のヒートポンプユニット暖房出力分担率 (-)
      Theta_SW_hs(ndarray): 1時間平均の温水暖房用熱源機の往き温水温度 (℃)
...
def calc_Q_GU_loss_HPU(Theta_SW_hs, Theta_TU_amb, r_HPU):
    ...
    Args:
      Theta_SW_hs(ndarray): １時間平均の温水暖房用熱源機の往き温水温度 (℃)
      Theta_TU_amb(ndarray): 1時間平均のタンクユニットの周囲空気温度 (℃)
...
def get_Q_out_hs_HPU(Q_out_hs, r_HPU):
    """1時間当たりの温水暖房用熱源機のヒートポンプユニット分断暖房出力 (12)

    Args:
      Q_out_hs(ndarray): 1時間平均の温水暖房用熱源機の暖房出力 (MJ/h)
      r_HPU(ndarray): 1時間平均のヒートポンプユニット暖房出力分担率 (-)
...
# ============================================================================
# E.3.7 温水暖房用熱源機の平均暖房出力及び最大暖房出力
# ============================================================================


def get_q_out_hs(Q_out_hs):
    """1時間平均の温水暖房用熱源機の暖房出力 (21)

    Args:
      Q_out_hs(ndarray): 1時間当たりの温水暖房用熱源機の暖房出力 (MJ/h)

    Returns:
      ndarray: 1時間平均の温水暖房用熱源機の暖房出力 (W)
...
def calc_Theta_TU_amb(Theta_ex, TU_place):
    ...
    if TU_place == '屋内':
        ...
        return 0.25 * Theta_ex + 0.75 * Theta_in  # (23-1)
    elif TU_place == '屋外':
        return Theta_ex  # (23-2)
    else:
...
def f_HPU_p(Theta_ex, h_ex, Theta_RW_hs, Theta_HP_out, q_HPU_out):
    ...
    Args:
      Theta_ex(ndarray): 外気温度 (℃)
      h_ex(ndarray): 外気相対湿度 (%)
      Theta_RW_hs(ndarray): 温水暖房用熱源機の戻り温水温度 (℃)
...
def f_HPU_q_min(Theta_ex, Theta_RW_hs, Theta_HPU_out):
    ...
    Args:
      Theta_ex(ndarray): 外気温度 (℃)
      Theta_RW_hs(ndarray): 温水暖房用熱源機の戻り温水温度 (℃)
      Theta_HPU_out(ndarray): ヒートポンプユニットの出湯温度 (℃)
...
def f_GU_q_lossHP(Theta_SW_hs, Theta_TU_amb):
    """ヒートポンプユニット運転時のガスユニットの熱損失を求める関数 (31)
