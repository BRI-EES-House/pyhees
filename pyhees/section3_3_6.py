...
def get_Wood_psi_j(heatbridge_j):
    ...
    :param heatbridge_j: 熱橋j
    :type heatbridge_j: dict(LinearHeatBridge_steel)
    :return: 熱橋jの線熱貫流率, heatbridge_jに計算結果を付加した辞書
    :rtype: float, dict(LinearHeatBridge_wood形式)
    """
...
def get_RC_psi_j(heatbridge_j):
    ...
    :type heatbridge_j: dict(LinearHeatBridge_steel)
    :return: 熱橋jの線熱貫流率, heatbridge_jに計算結果を付加した辞書
    :rtype: float, dict(LinearHeatBridge_rc形式)
    """
...
def calc_Steel_psi_j(heatbridge_j):
    ...
    :type heatbridge_j: dict(LinearHeatBridge_steel)
    :return: 熱橋jの線熱貫流率, heatbridge_jに計算結果を付加した辞書
    :rtype: float, dict(LinearHeatBridge_steel形式)
    """
...
def get_table_9(ExteriorThermalResistance, ColumnInterval):
    ...
    :param ColumnInterval: 柱見付寸法
                            'Over300'(300以上)または'Under300'(300未満200以上)または
                            'Under200'(200未満100以上)または'Under100'(100未満)
    :type ColumnInterval: str
    :return: 鉄骨造における一般部位の熱橋の線熱貫流率(psi_j)
    :rtype: float
    """
...
def calc_psi_F_j(foundation):
    ...
    :param foundation: 土間床等の外周部
    :type foundation: dict(Foundation)
    :return: 土間床等の外周部及び基礎等の線熱貫流率(W/m2K)
    :rtype: float
    """
...
def get_foundation_part(part_list, type_name):
    ...
    :param part_list: 土間床等の外周部に含まれる一般部分のリスト
    :type part_list: list<dict>
    :param type_name: 取り出したい一般部分のType
    :type type_name: str(GeneralPart_foundation>'Type')
    :return: 基礎等の熱損失を含めた土間床等の外周部の線熱貫流率(地盤面からの基礎等の底盤等上端の深さが1m以内)
    :rtype: float
    """
...
def get_psi_lower_1(R_1, R_2, R_3, R_4, H_1, H_2, W_1, W):
    ...
    :param R_1: 基礎等の立ち上がり部分の外気側に設置した断熱材の熱抵抗
    :type R_1: float
    :param R_2: 基礎等の底盤部分等の室内側に設置した断熱材の熱抵抗
    :type R_2: float
    :param R_3: 基礎等の底盤部分等の外気側に設置した断熱材の熱抵抗
    :type R_3: float
    :param R_4: 基礎等の立ち上がり部分の室内側に設置した断熱材の熱抵抗
    :type R_4: float
    :param H_1: 地盤面からの基礎等の寸法
    :type H_1: float
    :param H_2: 地盤面からの基礎等の底盤等上端までの寸法
    :type H_2: float
    :param W_1: 地盤面より下の基礎等の立ち上がり部分の外気側の断熱材の施工深さ
    :type W_1: float
    :param W: W_2及びW_3の寸法のうちいずれか大きい方の寸法
    :type W: float
    :return: 基礎等の熱損失を含めた土間床等の外周部の線熱貫流率(地盤面からの基礎等の底盤等上端の深さが1m以内)
    :rtype: float
    """
...
def get_psi_higher_1(R_1, R_4):
    ...
    :param W: W_2及びW_3の寸法のうちいずれか大きい方の寸法
    :type W: float
    :return: 基礎等の熱損失を含めた土間床等の外周部の線熱貫流率(地盤面からの基礎等の底盤等上端の深さが1mを超える)
    :rtype: float
    """
